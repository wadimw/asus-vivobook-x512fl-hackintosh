 ...
 *
 * Disassembly of SSDT-7-OptTabl.aml, Mon Dec 23 18:52:03 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00002249 (8777)
 *     Revision         0x01
 *     Checksum         0x3D
 *     OEM ID           "OptRef"
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    External (_SB_.PCI0.CBCN, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.CEDN, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.DVID, UnknownObj)    // (from opcode)
    ...
    External (_SB_.PCI0.LKDN, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LNKS, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LPCB.EC0_.BDIN, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LPCB.EC0_.BOTP, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LPCB.EC0_.BRAH, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LPCB.EC0_.CNTB, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LPCB.EC0_.CNTD, UnknownObj)    // (from opcode)
    ...
    External (_SB_.PCI0.RP05.DGCX, IntObj)    // (from opcode)
    ...
    Scope (\_SB.PCI0.RP05)
    {
        ...
        OperationRegion (RPCX, SystemMemory, Add (Add (\XBAS, ShiftLeft (ShiftRight (And (\_SB.PCI0.RP05._ADR (), 0x00FF0000), 0x10), 0x0F)), ShiftLeft (And (\RPA5, 0x0F), 0x0C)), 0x1000)
        ...
        PowerResource (PC05, 0x00, 0x0000)
        {
            ...
            {
                If (LEqual (\_SB.PCI0.RP05.TDGC, One))
                {
                    If (LEqual (\_SB.PCI0.RP05.DGCX, 0x03))
                    {
                        Store (One, _STA)
                        \_SB.PCI0.RP05.PEGP.GC6O ()
                    }
                    ElseIf (LEqual (\_SB.PCI0.RP05.DGCX, 0x04))
                    {
                        Store (One, _STA)
                        \_SB.PCI0.RP05.PEGP.GC6O ()
                    }

                    Store (Zero, \_SB.PCI0.RP05.TDGC)
                    Store (Zero, \_SB.PCI0.RP05.DGCX)
                }
                ...
                Or (\_SB.PCI0.LPCB.EC0.CNTD, 0x80, \_SB.PCI0.LPCB.EC0.CNTD)
            }

            Method (_OFF, 0, Serialized)  // _OFF: Power Off
            {
                Store (0xC9, \_SB.PCI0.LPCB.EC0.BRAH)
                If (LAnd (LEqual (\_SB.PCI0.LPCB.EC0.BOTP, Zero), LEqual (\_SB.PCI0.LPCB.EC0.CNTB, Zero)))
                {
                ...
                And (\_SB.PCI0.LPCB.EC0.CNTD, 0x38, \_SB.PCI0.LPCB.EC0.CNTD)
                If (LEqual (\_SB.PCI0.RP05.TDGC, One))
                {
                    CreateField (\_SB.PCI0.RP05.TGPC, Zero, 0x03, GUPC)
        ...
        Method (HDSM, 4, Serialized)
        {
            ...
            If (LOr (OPCI, LOr (SGCI, NBCI)))
            {
                ...
                If (LEqual (Arg2, 0x03))
                {
                    ...
                    If (LEqual (\_SB.PCI0.RP05.PEGP.SGST (), 0x0F))
                    {
                        Or (STS2, One, STS2)
                    }
                ...
                If (LEqual (Arg2, 0x1A))
                {
                    ...
                    If (ToInteger (FLCH))
                    {
                        Store (OMPR, \_SB.PCI0.RP05.PEGP.OPCE)
                    }
                    ...
                    If (ToInteger (DVSC))
                    {
                        If (ToInteger (DVSR))
                        {
                            Store (One, \_SB.PCI0.RP05.PEGP.GPRF)
                        }
                        Else
                        {
                            Store (Zero, \_SB.PCI0.RP05.PEGP.GPRF)
                        }
                    }

                    Store (\_SB.PCI0.RP05.PEGP.GPRF, SNSR)
                    If (LNotEqual (\_SB.PCI0.RP05.PEGP.SGST (), Zero))
                    {
                        Store (0x03, CGCS)
                    }
    ...
    Scope (\_SB.PCI0.RP05.PEGP)
    {
        Method (GC6I, 0, Serialized)
        {
            ...
            While (LNotEqual (\_SB.GGIV (0x04010015), One))
            {
                Sleep (One)
            }
        }
        ...
        Method (GC6O, 0, Serialized)
        {
            ...
            If (LEqual (\_SB.GGIV (0x04010015), One))
            {
                \_SB.PCI0.SGPO (Zero, 0x04010014, Zero, One)
                While (LNotEqual (\_SB.GGIV (0x04010015), Zero))
                {
                    Sleep (One)
                }

                \_SB.PCI0.SGPO (Zero, 0x04010014, Zero, Zero)
            }
            ...
            Store (One, \_SB.PCI0.CEDN)
        }

        Method (GETS, 0, Serialized)
        {
            If (LEqual (\_SB.GGIV (0x04010015), Zero))
            {
        ...
        Method (NGC6, 4, Serialized)
        {
            ...
            Switch (ToInteger (Arg2))
            {
                ...
                Case (0x03)
                {
                    ...
                    Store (Arg3, \_SB.PCI0.RP05.TGPC)
                    If (LOr (LNotEqual (ToInteger (GUPC), Zero), LNotEqual (ToInteger (DFGC), Zero)))
                    {
                        Store (ToInteger (DFGC), \_SB.PCI0.RP05.TDGC)
                        Store (ToInteger (GPCX), \_SB.PCI0.RP05.DGCX)
                    }
    ...
    Scope (\_SB.PCI0.RP05.PEGP)
    {
        ...
        Method (GPS, 4, Serialized)
        {
            ...
            Switch (ToInteger (Arg2))
            {
                ...
                {
                    Store ("GPS fun 19", Debug)
                    CreateBitField (Arg3, 0x02, PSTE)
                    Store (One, PSTE)
                    If (LEqual (\_SB.PCI0.LPCB.EC0.BDIN, Zero))
                    {
                        Store (One, \_SB.PCI0.LPCB.EC0.BDIN)
                        Notify (\_SB.PCI0.RP05.PEGP, 0xD1)
                        Store (0xC9, \_SB.PCI0.LPCB.EC0.BRAH)
                        And (\_SB.PCI0.LPCB.EC0.CNTD, 0xF8, \_SB.PCI0.LPCB.EC0.CNTD)
                        Or (\_SB.PCI0.LPCB.EC0.CNTD, 0x80, \_SB.PCI0.LPCB.EC0.CNTD)
                        Store (Zero, \_SB.PCI0.LPCB.EC0.CNTB)
                        Store (Zero, \_SB.PCI0.LPCB.EC0.BOTP)
                        Or (\_SB.PCI0.LPCB.EC0.ST8E (0x3B, 0xFF), 0x10, Local0)
                        \_SB.PCI0.LPCB.EC0.ST9E (0x3B, 0xFF, Local0)
                    }
                    ...
                    Switch (PSH0)
                    {
                        Case (Zero)
                        {
                            If (CPUT)
                            {
                                Store (0x0200, RETN)
                                Or (RETN, PSH0, RETN)
                                Store (\_SB.PCI0.LPCB.H_EC.LTMP, PDTS)
                            }
    ...
    Scope (\_SB.PCI0)
    {
        Device (WMI2)
        {
            ...
            Method (WMOP, 3, NotSerialized)
            {
                ...
                If (LEqual (Arg1, 0x02))
                {
                    If (LEqual (\_SB.PCI0.RP05.PEGP.DGPS, Zero))
                    {
                        Return (0x10)
                    }
                    Else
